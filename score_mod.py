"""
Adapted from https://github.com/MIT-CAVE/rc-cli/blob/main/scoring/score.py .
...
def evaluate(actual_routes, submission, cost_matrices, invalid_scores):
    '''
    ...
    **kwargs :
    ...
    scores = {'submission_score': 'x',
              'route_scores': {}, 'route_feasibility': {}}
        ...
        if route not in submission:
            ...
            try:
                ...
                if isinvalid(actual, sub):
                    ...
                    scores['route_scores'][route] = score(
                        actual, sub, cost_mat)
...
def erp_per_edit_helper(actual, sub, matrix, g=1000, memo=None):
    ...
    if len(sub) == 0:
        ...
        rest_sub = sub[1:]
        score1, count1 = erp_per_edit_helper(
            rest_actual, rest_sub, matrix, g, memo)
